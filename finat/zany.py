...
are not preserved under push-forward, :class:`~finat.physically_mapped.
PhysicallyMappedElement` constructs the matrix :math:`V` relating the
reference nodes to the push-forwards of the physical nodes, so that the
physical basis functions are obtained as :math:`M F^*(\hat\Psi)` with
:math:`M = V^T`.

Degrees of freedom are represented symbolically by
:class:`finat.functional.PhysicallyMappedFunctional` and processed
generically, without dispatching over FIAT functional types.  This
module supplies the two mapping-specific mixins that plug into the
entity-by-entity assembly loop:
...
from gem import Literal, Node, Power, Zero
from finat.physically_mapped import PhysicallyMappedElement, adjugate, determinant
